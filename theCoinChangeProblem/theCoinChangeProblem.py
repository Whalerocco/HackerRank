...
def getWays(n, c):
    ...
    for value in range(n+1): 
        ...
        for i in range(len(c)): # each row represents the nrWays for 1 smallest coin, 2 smallest coins etc..
            ...
            if i == 0: # Base case, if there's only one coin (the smallest)
                ...
                # a1 < a2 < a3
                # ways(n, [a1, a2, a3]) = ways(n-a3, [a1, a2, a3]) + ways(n-a2, [a1, a2]) + ways(n-a1, [a1])
                for j in range(i+1):   # this for loop is for summing values priorly calculated and stored in totalWays
    ...
    # Example: find get_ways(n=6, c=[1,3,5])
    # From generalized formula we can see that the total number of ways is equal to
    # the sum of the len(c) subproblems.

    # Coins ([1], [1,3], [1,3,5]) represented by rows in totalWays
    # The nr of ways for each "value" is stored stored in the columns e.g:
    ...
    # The answer is:
    # get_ways(6, [1,3,5]) == totalWays[coins index = 2][value = 6]
    # How do we get it? We realise that the generalized formula uses prior values in totalWays. 
    # Use 2 nested for loops to go through the whole matrix (all combinations of values and coins available)
    # Note: [1,5] is not a relevant coins combination to investigate. Just the combos where the largest coin is removed from the set each loop.
    # A third nested for loop is used to calculate the generalized formula (for each value/coin combo given by the nested for loops).
    # There are some base cases that are useful for filling the totalWays matrix when i == 0 (only smallest coin)
    # available and when value == 0. It's on these values that the rest of the totalWays matrix can be filled/calculated.
